...
export async function setupAiRoutes(fastify: FastifyInstance, services: ServiceContainer) {
	...
	const toolDefs = createToolDefinitions({ hotelContentService: services.hotelContentService });
	...
	fastify.post('/api/ai/chat', async (request, reply) => {
		try {
			...
			const result = streamText({
				model: getModel(),
				system:
					'You are a helpful hotel management assistant. You help hotel partners with bookings, reservations, room management, and other hospitality-related tasks. Be concise, professional, and helpful. When users ask about hotels, rooms, or availability, use the available tools to fetch real data.',
