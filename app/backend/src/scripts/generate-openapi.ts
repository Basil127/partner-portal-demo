...
async function generateOpenApi(): Promise<void> {
	...
	try {
		const additionalSpec = yaml.load(
			readFileSync(additionalSpecPath, 'utf8'),
...
function mergeOpenApiSpecs(
	base: OpenAPIV3_1.Document,
	additional: Partial<OpenAPIV3_1.Document>,
	...
	if (additional.tags) {
		// Merge tags by name
		const tagMap = new Map<string, OpenAPIV3_1.TagObject>(
			merged.tags?.map((t) => [t.name, t]) || [],
		);
	...
	if (additional.components) {
		...
		for (const key in additional.components) {
			...
			const extraComponents = (additional.components[componentKey] || {}) as Record<
				string,
				unknown
			>;
